...
def _render_working_urls(results: List[URLResult]) -> None:
    ...
    st.markdown(
        f'<div class="counter-badge">{len(working)} working URL{"s" if len(working) != 1 else ""}</div>',
        unsafe_allow_html=True,
    )

    working_text = "\n".join(r.url for r in working)
    st.code(working_text, language=None)
    copy_text = working_text
    df_working = pd.DataFrame(
        {"#": range(1, len(working) + 1), "URL": [r.url for r in working]}
    )

    copy_col, txt_col, xl_col = st.columns(3)
    with copy_col:
        components.html(
            clipboard_html(copy_text, btn_id="copy_working_urls"),
            height=52,
        )
    with txt_col:
        st.download_button(
